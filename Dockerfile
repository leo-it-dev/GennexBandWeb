...
RUN apt-get update && apt-get install -y unzip graphicsmagick ghostscript texlive-latex-recommended texlive-latex-extra

# Fix timezone
RUN rm /etc/localtime && ln -s /usr/share/zoneinfo/Europe/Berlin /etc/localtime

# Fix exiftool binary permissions
RUN chmod +x /app/package/backend/node_modules/exiftool-vendored.pl/bin/exiftool
